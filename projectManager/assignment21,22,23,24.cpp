...
class project2 {
	...
	public :void doneTask(string taskName){
		if(tcSize<=50){
			throw "Limit reached on Completed Task array";
		}
	...
	public :double showProgress(){
				if(tcSize!=0 && tpSize!= 0){
					return (tcSize/(tcSize+tpSize))*100;	
				}
				else if(tpSize==0){
					return 100.00;
				}
	...
	public :void addTask(string taskName){
			if(tpSize<=50){
			throw "Limit reached on pending Task array";
		}
			//cout<<"hii";
...
class IndustryProject: virtual public project2{
	...
	public: double getValuePerTask(){
		if(revenue==0){
			throw "revenue 0 Not a worth while project";
		}
	...
	public :IndustryProject operator+(IndustryProject& b){
	 ...
	 friend int daysSum(IndustryProject a,AcademicProject b);
};
class AcademicProject: public project2{
	...
	public: double getCreditsPerTask(){
		if(credists==0){
			throw"credits is 0 not a worth while project";
		}
		return (tpSize+tcSize)/credits;
	}
	friend int daysSum(IndustryProject a,AcademicProject b);
};
int daysSum(IndustryProject a,AcademicProject b){
	if(a.daysDueBy-b.daysDueBy<=0){
		throw "Critical situation: Industry Project cant be completed after academic\n";	
	}
	return a.daysDueBy-b.daysDueBy;
}
project2 compareP(project2 a,project2 b){
	if(a=="\0"||b=="\0"){
		throw "Compare not possible on Null Project\n";	
	}
...
project2 getLeastP(project2 *a,int aSize){
	if(*a=="\0"||aSize<1){
		throw "No Data available with given Parameters\n";	
	}
	project2 min = *a;
...
IndustryProject* searchp(IndustryProject *a,int aSize,string n){
	...
	if(e=="\0"){
		throw "Industry Project Not found\n";	
	}
	return e;
}
AcademicProject* searchap(AcademicProject *a,int aSize,string n){
	...
	if(e=="\0"){
		throw "Academic Project Not found\n";
	...
	return e;
			...
			int main(){
				...
				cout<<"12) get days left after academic project\n";
				cout<<"0) end\n";
				cin>>c;
				try{
					switch(c) {
					case 12:{
						...
						string pn2;
						cin>>pn2; 
						cout<<daysSum(*searchp(&pArr[0],y,pn1),*searchap(&aArr[0],y,pn2));	
						break;
					}
					case 11:{
						cout<<"enter name of Academic project";
